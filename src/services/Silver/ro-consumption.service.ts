...
import { calculateDateRangeForCym, calculateDateRangeForLYM, monthDifference } from '../../common/utils/jwt.helper';
...
function isRoIn(customer: any): customer is roIn {
  return (
      customer &&
...
@Injectable()
export class RoConsumptionService {
  constructor(
      ...
      let roInventory = await this.roInventoryModel.aggregate([
        { $match: { productId: ObjectId(productId), active: true } },
        {
          $project: {
            ...
            // L13: 1,
            // growthFactor: 1,
            // maxValue: { $max: ["$LYM", "$CYM", "$L13"] },
            // growthFactorNumber: { $toDouble: "$growthFactor" },
            // multipliedByGrowthFactor: {
            //     $multiply: [
            //         { $max: ["$LYM", "$CYM", "$L13"] },
            //         { $toDouble: "$growthFactor" }
            //     ]
            // }
          }
        },
  ...
  public async sumOfQualifiedDemand(productId): Promise<number> {
    try {
      // let roInventory = await this.roOrderModel.aggregate([
      //   {
      //     $match: {
      //       productId: ObjectId(productId),
      //       active: true,
      ...
      //     $group: {
      //       _id: null,
      //       totalOrderRecommendation: { $sum: "$cg.qty" }
      //     }
      //   }
      // ])
  ...
  public async createCgInventory(productId, qualifiedDemands, createdBy) {
    try {
  ...
  public async getRecommendationOrder(roId, productId): Promise<number> {
    try {
  ...
  public async getAvgWeeklyConsumption(roId, productId): Promise<number> {
    try {
      let avgWeeklyConsumption = 0;
      const currentDate = new Date();
  ...
  public async createConsumption(createConsumptionDto: CreateRoConsumptionDto, req: Request) {
    try {
      let date = await dateFormate(new Date());
      ...
      const consumption = new this.roConsumptionModel({ ...createConsumptionDto, createdAt: date, inventoryId: inventoryId, createdBy: authInfo._id });
      if (consumption.save()) {
        return new GetRoConsumptionInfoDto(consumption)
      }
    } catch (e) {
      throw new InternalServerErrorException(e)
    }
  ...
  async getAllConsumption(paginationDto: FilterPaginationRoConsumptionDto, req: Request): Promise<any> {
    try {
  ...
  async getConsumptionInfo(id: string): Promise<GetRoConsumptionInfoDto> {
    try {
      ...
      return new GetRoConsumptionInfoDto(data[0]);
    } catch (e) {
      throw new InternalServerErrorException(e)
    }
  };

  async UpdateConsumption(updateConsumptionDto: UpdateRoConsumptionDto, req: Request): Promise<any> {
    try {
      let date = await dateFormate(new Date());
      let authInfo = await getAuthUserInfo(req.headers);
      ...
      const updateInventory = await this.roInventoryModel.findByIdAndUpdate({ _id: findInventoryExist._id }, { ...inventoryObject }, { new: true, useFindAndModify: false })
      ...
      await this.roConsumptionModel.findByIdAndUpdate({ _id: updateConsumptionDto.consumptionId }, { ...updateConsumptionDto, inventoryId: findInventoryExist._id }, { new: true, useFindAndModify: false });
    }
    catch (e) {
      throw new InternalServerErrorException(e)
    }
  ...
  async importConsumption(req: Request, createConsumptionDto: ImportRoConsumptionDto[]) {
    try {
      let date = await dateFormate(new Date());
      let errorArray = [];
      let consumptionArr = [];
      ...
      const mappedArray = await Promise.all(createConsumptionDto.map(async (consumption: any) => {
            ...
            if (cpExist && productExist) {
              ...
              if (inventoryExist){
                if(consumption.updatedInventory == "NO"){
                  const consumptionAdd = new this.roConsumptionModel({
                    ...consumption, 
                    createdAt: await dateFormate(consumption.date),
                    date: await dateFormate(consumption.date), 
                    roId: cpExist._id,
      ...
      return new GetRoConsumptionInfoDto({ mappedArray: mappedArray, errorArrray: errorArray });
    } catch (e) {
      console.error(e);
    }
  };
  
  async updateConsumptionStatus(id: string, updateStatusConsumptionDto: updateStatusRoConsumptionDto): Promise<updateStatusRoConsumptionDto> {
    try {
      ...
      if (!findConsumption) {
        ...
        if (RO) {
          throw new BadRequestException("RO inactive");
        }
  ...
  async getConsumptionDropDown(req: Request): Promise<any> {
    try {
      ...
      const channelPartner = await this.roConsumptionModel
        ...
        .populate({
          path: "roId",
          select: "contactPersonName roName", // Only include needed fields
          model: "RoMaster",
        })
        .populate({
          path: 'productId',
  ...
  //     const channelPartner = await this.roConsumptionModel.aggregate([
  ...
  async getRoInventoryList(req: Request,addRoConsumptionDto:AddRoConsumptionDto): Promise<any> {
    try {
      const authInfo = await getAuthUserInfo(req.headers);
