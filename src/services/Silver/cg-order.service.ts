...
export class CgOrderService {
  ...
  async getAllOrder(paginationDto: FilterPaginationCgOrderDto, req: Request): Promise<any> {
    try {
  ...
  async updateOrder(id:string,updateOrderDto:UpdateCgOrderDto) {
    try {
        ...
        if (findOrder.wip.status == true && findOrder.wip.stage == "PENDING") {
            ...
            if (updateOrderDto.isChangeQty) {
                ...
                await this.cgOrderModel.findByIdAndUpdate({ _id: findOrder._id }, 
               { $set:{
        ...
        if (updateOrderDto.partcialDispatch && updateOrderDto.partcialDispatch == true && updateOrderDto.isChangeQty == false) {
            ...
            if (updateOrderDto.changeQty == findOrder.qty || updateOrderDto.changeQty > findOrder.qty) {
                throw new BadRequestException("Partial dispatch QTY can not be equal to Full QTY");
            }
            const maxPartialUniqueNumberResult = await this.cgOrderModel.aggregate([
                {
                    $match: {
  ...
  async dashBoardOrder(dashboardOrderDto: DashboardCgOrderDto, req: Request): Promise<any> {
    try {
      let activeCondition = { active: true };
  ...
  async getOrderDropDown(req: Request): Promise<any> {
    try {
