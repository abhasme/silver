...
import { getAuthUserInfo, dateFormate, flagFunction, multiSelectorFunction } from "src/common/utils/jwt.helper";
...
export class RoOrderService {
  ...
  // Update Central godown Inventory behalf of RO grn
  ...
  public async updateCgInventory(productId, createdBy, updateQty): Promise<any> {
    try {
  ...
  public async addDataInxlsxFile(dataArray, range): Promise<any> {
    try {
  ...
  async getAllOrder(paginationDto: FilterPaginationRoOrderDto, req: Request): Promise<any> {
    try {
  ...
  async updateOrder(id: string, updateOrderDto: UpdateRoOrderDto, req: Request): Promise<any> {
    try {
      const authInfo = await getAuthUserInfo(req.headers);
      ...
      if (findOrder.in_trasit.status == true && findOrder.in_trasit.stage == "PENDING") {
        ...
        if (updateOrderDto.isChangeQty) {
          ...
          if (checkCgStock) {
            if (updateOrderDto.changeQty > checkCgStock.onHandStock && updateOrderDto.partcialDispatch == true) {
              throw new BadRequestException("Stock is insufficient. Dispatch quantity should not be more than on hand stock.");
            } else if (qty > checkCgStock.onHandStock && updateOrderDto.partcialDispatch == false) {
              throw new BadRequestException("Stock is insufficient. Dispatch quantity should not be more than on hand stock.");
            }
          } else {
            throw new BadRequestException("Cg inventory not exist");
          }
        ...
        condition = true;
        await this.roOrderModel.findByIdAndUpdate(
          findOrder._id,
          {
            tog: inventory.tog,
      ...
      if (updateOrderDto.partcialDispatch && updateOrderDto.partcialDispatch == true && updateOrderDto.isChangeQty == false) {
        ...
        if (Number(qty - changeQty) != 0 && qty >= changeQty) {
          ...
          await this.roOrderModel.findByIdAndUpdate(
            findOrder._id,
            { ...obj, qty: qty, "wip.partialDispatchQty": qty },
            { new: true, useFindAndModify: false }
          );
        }
      } else {
      ...
      let onHandStatus = 0;
      let flag = "WHIGHT";
      //------------------------------------------------------------ For Grn and dispatch condition -------------------------------------------//
      ...
      if (grn) {
        ...
        if (isNaN(orderRecommendation)) {
          ...
          orderRecommendationStatus = Math.abs(
            (Number(orderRecommendation) * 100) / findInventory.tog
          );

        }
        let onHandStatusNumber = Math.abs(
          (Number(onHandStock) * 100) / findInventory.tog
        );
        ...
        flag = await flagFunction(findInventory.tog, onHandStock)
        ...
        // For net orderRecommendationStatus
        let orderRecommendationStatus = 0;
        if (isNaN(orderRecommendation)) {
          orderRecommendationStatus = 0;
        } else {
  ...
  async dashBoardOrder(dashboardOrderDto: DashboardRoOrderDto, req: Request): Promise<any> {
    try {
      const authInfo = await getAuthUserInfo(req.headers);
      const user = await this.userModel.findOne({ _id: ObjectId(authInfo._id), userType: "cg" })
      let cpObj = {}
      ...
      cpObj = { roId: { $in: cpArr } }
      let activeCondition = { active: true };
      if (dashboardOrderDto.roId) { condition = { roId: ObjectId(dashboardOrderDto.roId) } }
      if (dashboardOrderDto.active == false) { activeCondition = { active: false } }
      const order = await this.roOrderModel
        .aggregate([
  ...
  async getOrderDropDown(req: Request, addRoIdInfo: AddRoIdInfo): Promise<any> {
    try {
      const authInfo = await getAuthUserInfo(req.headers);
      let roObj = {}
      if (addRoIdInfo.roId) {
